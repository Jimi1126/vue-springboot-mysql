...
Customer.prototype = {
      ...
      methods: {
        ...
        cellClickEvent: (row, col, cell, e) => {
          if (col.property == "name") {
            that.detailDialogComponent._vue.$data.provinces = ic_sms.address.filter((add) => { return add.parentid == row.country });
            that.detailDialogComponent._vue.$data.citys = ic_sms.address.filter((add) => { return add.parentid == row.province });
        ...
        editBtnEvent: (index, row) => {
          that.editDialogComponent._vue.$data.provinces = ic_sms.address.filter((add) => { return add.parentid == row.country });
          that.editDialogComponent._vue.$data.citys = ic_sms.address.filter((add) => { return add.parentid == row.province });
          row.province = +row.province;
          row.city = +row.city;
        ...
        displayFormat: function (row, column, cellValue, index) {
          ...
          switch (column.property) {
            case "city":
              obj = ic_sms.address.filter((item) => { return item.areaid == cellValue })[0] || {};
              return obj.areaname || "";
            case "origin":
              obj = ic_sms.enum["客户来源"].filter((item) => { return item.code == cellValue })[0] || {};
              return obj.name || "";
            case "trade":
              obj = ic_sms.enum["行业"].filter((item) => { return item.code == cellValue })[0] || {};
              return obj.name || "";
            case "status":
              obj = ic_sms.enum["客户状态"].filter((item) => { return item.code == cellValue })[0] || {};
              return obj.name || "";
            default:
              return cellValue;
          }
  ...
  loadTable: function () {
    ...
    $api("/customer/selectByExampleAndPage", JSON.stringify(param), function (data) {
      $.ic.loadUI.hide();
      if (data.status == "fail") return alert(data.data.errMsg);
